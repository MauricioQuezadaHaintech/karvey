...
- **`approvals.prod = { por, fecha, ref: D-NN }`** — mandatory in `karvey-deploy` before the merge to the production branch, committed in the repo, never delegated to an agent. Keeps the prod approval in git history where the platform cannot enforce required reviewers (#5).
...
### Changed
- **Approvals record who and where** (`approvals.<phase>.por`, `rol: human | ceo-delegate`, `fecha`, `ref: D-NN`) — applied at every gate through `phase-close.md` and `karvey-requirements` (#10).
