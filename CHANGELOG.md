...
- **`project.json:standards`** field (`dir` + `by_layer`) so phases know which standard to load; fallback chain to `_index.md` and, failing that, to "no standard found → ask".
