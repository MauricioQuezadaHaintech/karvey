...
- Date: 2026-09-26
- Commits included: 88 up to the reviewed commit; every one carries `Karvey-Change: wave2-structural` (checked
  with `git log --format=%B` over `merge-base..HEAD`; the release gate re-checks it at deploy)
...
(`evidence.jsonl:39`). This QA — the manual runs, subagent reviewers for dimensions 1–4, the second opinion and the
two QA judges — found **30 defects** in the change (bugs and spec-gaps); **29 are fixed** with regression tests
that were red on the pre-QA scripts (`evidence.jsonl:30`: 66 of 69 red, the other three guard against
over-matching) — 28 incidents, BUG-49 .. BUG-77 except BUG-66 (the impl finding F-05). **One High stays open for the owner: F-61** (whether one project-wide
...
`qa/suppressions.json` with its reason and scope (`validate-suppressions`: 8 entries, 0 problems) — B404/B603/B607
...
### 2. Code errors
All fixed with red-first tests (`evidence.jsonl:30`): zone lost on fractional seconds (BUG-54); tracebacks with
...
over all 51 checks (`evidence.jsonl:36`). The D3 reviewer also compared every script command a skill names with
...
A Wave 1 `spec.json` and `project.json` validate unchanged in advisory mode: 0 errors, 2 warnings
(`evidence.jsonl:33`). A spec without `lane` stays `legacy` with granular gates in 3.13; under strict (4.0) it is
...
New in the diff: `KARVEY_ID_LOCK_WAIT_S` (lock wait of `karvey-id.py`, default from `defaults.json`). No `.env`,
Dockerfile or pipeline in this repository consumes it; it has a fallback.
...
- **fiscal: fail** on revision 1 — 15 rows (F-46 .. F-60): 14 accepted and resolved by this revision, 1 rejected
  (F-60: the review never claimed a CI run).
...
- [x] Tests: pass at `9cb18d4` — unit `evidence.jsonl:34`, regression `:35`, guard tables `:40`, hook commands `:41`, page `:38`; red on the pre-QA scripts `:30`
- [x] Coverage: 97/97 (pass, mode warn) — `evidence.jsonl:39`
