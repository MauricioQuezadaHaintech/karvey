...
**83 PASS · 0 FAIL · 5 PENDING (manual).** The 9 MODIFIED Wave 1 requirements are green in `traceability.md`
...
| REQ-W2-008 | The retro works on the method's artifacts | manual: retro-from-metrics.md | PENDING (manual) |
| REQ-W2-009 | Actions are followed up; per-person analysis is optional | manual: retro-from-metrics.md | PENDING (manual) |
...
| REQ-W2-041 | Grill asks in batches | manual: grill-batches.md | PENDING (manual) |
...
| REQ-W2-063 | QA runs the suite or cites the exact run | manual: QA behaviour: checked by the fiscal at this change's QA | PENDING (manual) |
...
| REQ-W2-078 | A regression proposes the rollback and opens the incident | manual: deploy-postdeploy.md (rollback question); the script half (regression result, rollback shown, deploy-record) is PASS in test_postdeploy.py | PENDING (manual) |
...
| F-08 | emergent | 5 requirements verified only by manual scripts / QA behaviour: PENDING, not PASS | open |
