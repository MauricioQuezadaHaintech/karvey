...
| EoP | Notification destination | A changed `target` redirects notices (F-32) | `://` refused; `notify-check` asks for confirmation on change (REQ-W1-097) | — |
...
| `test_notify_check.py` | unchanged → 0; changed → 10; `--confirm`; `detail` default counts | 097, 098 |
