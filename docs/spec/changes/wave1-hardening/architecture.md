...
- a `git push` in every recognised form (explicit refspec, `@`, `HEAD`, implicit upstream or configured push
  ...
  target repository. `--all`, `--mirror` and a delete name no single commit and block;
