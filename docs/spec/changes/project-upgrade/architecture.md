...
| A-08 | Branch base and fetch | the skill fetches; the tool never fetches and bases on `origin/<integration>` when the local ref exists, else the local branch | the tool fetches; always the local branch |
