...
- **REQ-ADP-031** — THE page `docs/karvey.html` SHALL be authored in **English by default** (what renders with JavaScript disabled) AND SHALL offer a language switch to **Spanish, Portuguese, German and Chinese (Simplified)**, all embedded in the same self-contained file; the chosen language SHALL be remembered per viewer when the browser allows it and SHALL be selectable with `?lang=en|es|pt|de|zh`. ON a first visit with neither, THE page SHALL select the browser's primary language when it is one of the five, and English otherwise; THE tab title SHALL follow the selected language.
