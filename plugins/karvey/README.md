...
- **Hooks** (python ≥ 3.9): `hooks/hooks.json` registers the session hook and the dispatcher `hooks/karvey-hook.sh` (approval, protect-paths, prod-gate on by default; git-flow and plan-gate opt-in through `project.json:enforcement`; spec-write validator; subagent-prompt check). The statusline is installed by hand. See `hooks/README.md`.
