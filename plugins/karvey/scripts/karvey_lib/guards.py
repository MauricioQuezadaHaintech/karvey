...
def _evaluate_candidate(ctx, c, deadline):
    ...
    mblock, mlines = manifest_verdict(ctx, root, wc, base, head, cid, deadline, released)
    ...
    return Decision.allow(stdout=warn + ["[karvey] prod-gate ALLOW change=%s by=%s ref=%s commit=%s"
                                         % (cid, res.get("by"), res.get("ref"), released[:12])] + mlines,
                          record=rec, audit=True)
...
def manifest_verdict(ctx, root, wc, base, head, cid, deadline, released=None):
    """``(block Decision | None, stdout lines)``: warn → allow + a line; blocking → block on a non-pass or
    not computable manifest, and every manifest change must pass ``check_prod`` for the released
    commit (REQ-W2-046, 047; D-35)."""
    ...
    for c in man["changes"]:
        if c["id"] == cid:
            continue
        try:
            if not tool.check_prod(root, c["id"], sha=released)["ok"]:
