...
Commands:
  ...
  approve <change> <phase> --by --role human|ceo-delegate|auto --ref [--date] [--write-spec] [--sha REV]
                                        [--manifest [--base REF] [--pr-body FILE]]  (prod: D-37)
...
def _approve_prod_manifest(args, root, by, date, ref, head_sha):
    ...
    try:
        man = mf.release_manifest(root, base, head_sha)
...
def cmd_approve(args, root):
    ...
    if key == "prod":
        ...
        if getattr(args, "manifest", False):
            return _approve_prod_manifest(args, root, by, date, ref, head_sha)
        marker, scope, reasons = approval.find_valid(root, args.change, kinds=("prod",), ttl_min=reviewed_ttl(root),
...
def cmd_approve_gate(args, root):
    ...
    head_sha = None
    if write_prod:  # D-35: the release gate binds prod to the approved head commit, like approve prod
        head_sha = resolve_commit(root, getattr(args, "sha", None) or "HEAD")
        if head_sha is None:
            raise Refused("cannot bind the production approval to a commit: %r is not a commit of this repository "
                          "(D-35; pass --sha with the head the human approved)" % (args.sha or "HEAD"),
                          code="state.prod_sha")
...
def build_parser():
    ...
    ag.add_argument("--sha", help="release gate with prod: the head commit the human approved (default HEAD; D-35)")
