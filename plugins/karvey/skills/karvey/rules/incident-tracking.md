...
> complementary to the team's tracker (`management-adapters`[^r-management-adapters]), not a replacement. The tracker holds *work/tasks*; this tracker keeps
...
A `bug`-type finding (see `iteration-loop`[^r-iteration-loop]) is promoted to a `BUG-NN` here by `karvey-iterate`.
...
- **vs the team's tracker:** if one is configured, the `BUG-NN` references its tracker item and vice-versa. Status changes are mirrored at the phase-close ritual (`phase-close`[^r-phase-close]).
