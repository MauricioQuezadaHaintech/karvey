...
- Blocks pushes that can reach production without naming one commit: a wildcard or matching (`:`) refspec, a configured mirror or `push.default matching`, and a push that sends different commits to two production branches; a tag cannot stand in for the pushed branch. <!-- guard-case: pg6-01-wildcard-refspec-into-main, pg6-02-matching-colon-refspec, pg6-03-configured-mirror, pg6-04-configured-push-default-matching, pg6-06-second-production-destination, pg6-07-configured-wildcard-push-refspec, pg6-05-tag-shadows-the-pushed-branch -->
- Reads push options as git does: a `refs/*` wildcard, `-o` inside a short cluster, an abbreviated long option, a remote name with a slash, `--no-dry-run` after `--dry-run` and the remote named by `--repo` are resolved, and an unknown long option blocks. <!-- guard-case: pg6-13-refs-wildcard-refspec, pg6-14-push-option-cluster-is-not-a-dry-run, pg6-15-abbreviated-mirror-option, pg6-16-unknown-long-option, pg6-17-remote-name-with-a-slash, pg6-18-dry-run-cancelled-by-no-dry-run, pg6-19-repo-option-names-a-mirror-remote, pg6-20-repo-option-names-a-wildcard-remote -->
- Blocks a deferred merge unless the host is bound to the approved commit: `gh pr merge --auto` needs `--match-head-commit <sha>`, `glab mr merge` needs `--sha <sha>`, and `az … --auto-complete` always blocks. <!-- guard-case: pg6-08-gh-auto-merge-unbound, pg6-10-gh-auto-merge-bound-to-another-commit, pg6-12-glab-merge-without-sha, pg6-11-az-auto-complete-deferred -->
- Allows a deferred gh merge bound to the approved commit. <!-- guard-case: pg6-09-gh-auto-merge-bound-to-the-approved-commit -->
- Without python it blocks every PR merge and every push that names a production branch, has no refspec, or has a wildcard or matching (`:`) refspec (fail closed); it never reads the ledger, so the commit and expiry checks do not apply there. <!-- guard-case: pg5-02-commit-after-the-approval, pg6-01-wildcard-refspec-into-main, pg6-02-matching-colon-refspec -->
