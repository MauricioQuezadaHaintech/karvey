...
- When the phase's gate was **approved**, record who and where on that approval: `approvals.<phase>.por`, `rol` (`human` | `ceo-delegate`), `fecha`, `ref` (the `D-NN` of the decision log where the approval is written). An approval without `ref` in a multi-agent project is incomplete (see `multi-agent.md` §4).
