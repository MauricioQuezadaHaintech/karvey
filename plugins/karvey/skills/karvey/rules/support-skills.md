...
| `karvey-judges` | Independent judges | Before the requirements, architecture and qa gates: one clean-context subagent per lens reads the phase's closed inputs and reports cited findings into `findings.md`; advisory by default, cost logged (`judges`[^r-judges]). | — (Karvey) |
...
| `karvey-team` | Team layer (**optional**) | Set up and run a team of agents (roles, manifests, boards, census, relay) and **measure what it costs**. Opt-in: Karvey is complete with one agent, and a team is expensive — read `team`[^r-team] first. | — (Karvey) |
...
| `karvey-standards` | Standards uplift | Distill the team's engineering golden paths (db/backend/frontend…) from the **real system** into the team's standards repo (`project.json:standards`), in the `engineering-standards`[^r-engineering-standards] format. Re-runnable to refresh. Never writes into the public plugin. | — (Karvey) |
...
- They do not run the knowledge sync: it runs at archive and on demand only (`knowledge-sync`[^r-knowledge-sync]).
...
[^r-judges]: judges.md — context only, not opened.
