...
description: Cross-cutting decision log for the Karvey method — a single numbered registry (D-NN business, C-NN direction) that changes cite, plus `cross`, which checks a question against the log BEFORE declaring it blocked. Triggers include "karvey decisions", "registro de decisiones", "decision log", "D-NN", "está decidido", "is this decided", "bloqueado por decisión", "blocked on a decision".
...
Both live in **one file per period** in the ops repo (`{ops_repo}/decisions/`), numbered **once,
never reused, never renumbered**. A decision is immutable: it is superseded by a later one that cites
...
1. Resolve the ops repo (`project.json:ops_repo`, else `spec_repo`) and the next free number in the
   series. **Scan the whole `decisions/` directory**, not just the current file.
...
1. Search `{ops_repo}/decisions/` for the question's subject — by concept, not only by keyword.
2. Search the **product/offer material** too (pricing, plans, commercial docs). Roughly half of what
   ...
   directories is how the 13-out-of-14 happened.
...
4. **Result:**
   ...
   - Not found → the block is written in this exact form: *"searched decisions/ and the offer material,
     no answer exists for X"*. A block written any other way is not written.
